...
* The latest verified designer run used model `gpt-5-mini`, used 4 model-planned semantic-search focuses, produced 202 RDF triples, 22 RDFS classes, and 23 RDF properties, and was verified through a clean non-DnD PDF end-to-end run.
...
* The latest verified importer run used model `gpt-5-mini`, inspected ontology from Fuseki, used 4 model-planned import batches, produced 108 instance RDF triples and 310 combined triples, and was verified through viewer workflow checks, FastAPI endpoint checks, Turtle export parsing, and local RDF/SPARQL checks.
...
* The non-DnD PDF validation proves the architecture is domain-neutral, but it is still a proof of concept. The current generated semantic web intentionally does not cover every fact in the 200+ page notebook because the prompts, class/property limits, retrieval focus count, and importer batch count are tuned for compact first-pass output.
* Future scale-up work should improve coverage and graph richness with stronger models, more explicit comprehensive-coverage prompts, higher retrieval and importer iteration limits, section/page-level coverage tracking, better PDF preprocessing for headings/equations/tables/code, schema refinement passes, and graph-slice retrieval backed by Fuseki and semantic-web embeddings.
...
* End-to-end validation from clean generated outputs succeeded on 2026-06-01 against `./data/main.pdf`, a probability, statistics, and data-science notebook. Viewer questions covered probability distributions, Poisson facts, CLT/LLN assumptions, and sample-mean reasoning, and Turtle export parsed successfully.
