...
* The latest verified designer run used model `gpt-5-mini`, used 4 model-planned semantic-search focuses, produced 165 RDF triples, 16 RDFS classes, and 21 RDF properties, and was verified through a clean non-DnD CSV-aware end-to-end run against semantic-web and ontology markdown files plus a triplestore CSV.
...
* The importer now supports CSV-aware deterministic import: CSV files are summarized as profiles, the model plans constrained row-to-RDF mapping JSON, Python validates mappings against the generated ontology and CSV headers, and Python loops over all CSV rows to emit RDF instances.
...
* The latest verified importer run used model `gpt-5-mini`, inspected ontology from Fuseki, produced 241 instance RDF triples and 406 combined triples, including 157 deterministic CSV triples from one validated CSV mapping, and was verified through viewer workflow checks, FastAPI endpoint checks, Turtle export parsing, and local RDF/SPARQL checks.
...
* The viewer now uses class-instance aggregate counts for count-style class questions and exact subject-label fact lookup for named-entity questions before generic relevance search. Viewer answers should be end-user-facing and avoid database/RDF implementation wording unless the user asks technical implementation questions.
* End-to-end validation from clean generated outputs succeeded on 2026-06-01 against semantic-web and ontology markdown files plus `./data/commonly seen triplestores.csv`. Viewer questions covered 11 triplestores, 4 open-source triplestores, Apache Jena TDB APIs/protocols, GraphDB maintainer/license, commercial-license triplestores, and Virtuoso's key feature. Turtle export parsed successfully with 406 triples, and `uv run pytest` reported 74 passed and 2 skipped after viewer count/relevance updates.
